...
$(document).ready(function(){
	$("#btnSearch").on("click", function(){
		document.guestForm.searchValue.value = $("#searchValue").val();
		document.guestForm.curPage.value = "1";
		document.guestForm.action = "/admin/adminMain";
...
{
   document.guestForm.curPage.value = curPage;
   document.guestForm.action = "/admin/adminMain"
   document.guestForm.submit();
}
</script>
		...
		<div class="info-section">
			<div class="tabs">
				<div class="tab active">
					<h2 class="space_management_title">게스트 계정 관리</h2>
				</div>
			</div>
									...
									<div class="manegementComment-content">
										...
										<c:if test='${!empty list}'>
												...
												<tbody>
													<c:forEach var="guest" items="${list}" varStatus="status">
														<tr>
															...
															<td>${guest.guestNickname}</td>
															<td>${guest.guestTel}</td>
															<td class="status-cell">
																<select id="guestStatus" name="guestStatus" class="status" onchange="changeStatus(this, '${guest.guestEmail}')" data-prev-value="${guest.status}"  ${guest.status == 'N' ? 'disabled' : ''}>
																	<option value="Y" ${guest.status == 'Y' ? 'selected' : ''}>정상</option>
																	<option value="N" ${guest.status == 'N' ? 'selected' : ''}>탈퇴</option>
																	<option value="S" ${guest.status == 'S' ? 'selected' : ''}>정지</option>
																</select>
															</td>
															<td>${guest.regDate}</td>
															<td>${guest.modiDate}</td>
															<td>${guest.delDate}</td>
														</tr>
													</c:forEach>
												</tbody>
											...
											<script type="text/javascript">
												function changeStatus(selectElement, guestEmail) {
												    ...
												    if (confirm("유저의 상태를 변경하시겠습니까?")) {
												        // AJAX 요청 실행
												        updateGuestStatus(guestEmail, newValue, $select);
												    } else {
												        // 사용자가 취소하면 기존 값으로 복원
												...
												function updateGuestStatus(guestEmail, newStatus, $select) {
												    $.ajax({
												        type: 'POST',
												        url: '/admin/updateGuestStatus',
												        data: {
